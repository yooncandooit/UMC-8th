...
const MyPage = () => {
    ...
    const editMutation = useMutation({
        ...
        onSuccess: () => {
            setEditMode(false);
            // 내 정보 refetch
            window.location.reload(); // 간단하게 새로고침으로 반영
        },
    });
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-black p-4 relative">
            ...
            <div className="flex flex-col items-center mb-8">
                {editMode ? (
                    ...
                    <>
                        <img
                            src={data?.data.avatar || "https://cdn-icons-png.flaticon.com/512/149/149071.png"}
                            alt="avatar"
                            className="w-32 h-32 rounded-full object-cover mb-4 border-2 border-white"
                        />
                        <div className="flex flex-col items-center">
                            <div className="text-white text-2xl font-bold mb-2">{data?.data.name}</div>
                            <div className="text-white text-lg mb-2">{data?.data.bio}</div>
                            <div className="text-white text-base mb-2">{data?.data.email}</div>
                        </div>
                        <button
